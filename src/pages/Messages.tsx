...
const ConversationItem = ({ conversation, isSelected, onClick, isTyping }: ConversationItemProps) => {
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        'w-full flex items-center gap-3 p-4 hover:bg-secondary/60 transition-all duration-300 group animate-conversation-slide-in rounded-2xl mx-1 my-0.5',
        isSelected && 'bg-gradient-to-r from-primary/10 to-accent/5 shadow-md shadow-primary/10 ring-1 ring-primary/20'
      )}
    >
  ...
  return (
    <MainLayout>
      <div className="h-[calc(100vh-80px)] md:h-screen flex">
          ...
          <div
            className={cn(
              'w-full md:w-80 lg:w-96 border-r border-border/30 bg-card/30 backdrop-blur-sm flex flex-col',
              selectedConversation && 'hidden md:flex'
            )}
          >
          {/* Header */}
          <div className="p-4 border-b border-border/30 flex items-center justify-between bg-gradient-to-r from-background/90 to-card/50 backdrop-blur-xl">
          ...
          <div className="flex-1 overflow-y-auto">
            {conversationsLoading ? (
              <div className="space-y-2 p-4">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="flex items-center gap-3 p-4 rounded-2xl" style={{ animationDelay: `${i * 100}ms` }}>
        ...
        >
          {selectedConversation ? (
            <>
              {/* Chat Header */}
              <div className="flex items-center justify-between p-4 border-b border-border/30 bg-card/30 backdrop-blur-xl">
              ...
              <div className={cn("p-3", chatTheme?.backgroundGradient)}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className={cn(
                    "flex-1 flex items-center gap-2 rounded-2xl border bg-background/80 backdrop-blur-sm px-4 py-2.5",
                    "focus-within:ring-2 focus-within:ring-primary/50 focus-within:shadow-lg focus-within:shadow-primary/10 transition-all duration-300",
                    chatTheme ? "border-white/20" : "border-border/50"
                  )}>
                    <Input
                      value={newMessage}
            ...
            <div className="flex-1 flex items-center justify-center text-muted-foreground animate-fade-in">
              <div className="text-center">
                <div className="w-24 h-24 mx-auto mb-6 rounded-3xl bg-gradient-to-br from-primary/10 to-accent/10 border border-border/30 flex items-center justify-center shadow-xl shadow-primary/5">
                  <Send className="w-10 h-10 text-primary/60" />
                </div>
                <h3 className="font-semibold text-xl mb-2">Your Messages</h3>
                <p className="text-sm text-muted-foreground/80">Send private messages to your friends</p>
              </div>
            </div>
          )}
          </div>
